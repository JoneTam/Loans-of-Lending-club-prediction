...
def employment_Length_to_numeric(df, column1):
    df[column1] = (
        df[column1]
        .replace(
            {
...
def filter_column_range_0_to_100(df, column):
...
def calc_risk(df, column1, column2):
...
def replace_rare_categorical_values(df, threshold=0.3):
    ...
    for column in df.select_dtypes(include=["object"]):
        ...
        # Identify values to replace with "Rare feature"
        rare_values = value_counts[value_counts < threshold_count].index

        # Replace the rare values with "Rare feature" while keeping NaN values as is
